...
import static com.samagra.commons.utils.RemoteConfigUtils.AUTH_BASE_URL;
...
//        UpdateDriver.init(this);
        ...
        Network.Companion.init(new ModuleDependency() {
            ...
            public String getBaseUrl(@NotNull String type) {
                return RemoteConfigUtils.getFirebaseRemoteConfigInstance().getString(AUTH_BASE_URL);
            }
...
//        Realm.init(this);
    ...
    private void setupNetworkConfig(FirebaseRemoteConfig firebaseRemoteConfig, String authToken) {
        String loginServiceUrl = BuildConfig.DEBUG ? BuildConfig.DEFAULT_AUTH_BASE_URL
                : firebaseRemoteConfig.getString(AUTH_BASE_URL);
...
//        appServiceInterceptors.add(new HasuraAuthorizationInterceptor());
    ...
    private void initODK(String odkServerUrl) {
        ODKProvider.INSTANCE.init(getCurrentApplication());
        try {
            String settings = IOUtils.toString(getResources().openRawResource(R.raw.settings));
            if (odkServerUrl.isEmpty() || settings.isEmpty()) {
                return;
            }
            JSONObject settingsJson = new JSONObject(settings);
            try {
                settingsJson.getJSONObject("general").put("server_url", odkServerUrl);
            } catch (JSONException e) {
                Timber.e(e);
            }

            ODKProvider.INSTANCE.getOdkInteractor().setupODK(
                    settingsJson.toString(),
                    true,
                    new ODKProcessListener() {
                        @Override
                        ...
                        @Override
                        public void onProcessingError(@NonNull Exception exception) {
                            Timber.e(exception, "Form Module could not be initialised correctly.");
            ...
            Timber.e(e, "Form Module could not be initialised correctly.");
